...
class TokenWallet{
    ...
    constructor(wallet,client){
        ...
        this.permitClient = new PermitClient(wallet,{apiKey:permitApiKey});
    }

    // forwarderClient needs to be created using async factory method, permitClient doesn't
    async factory(provider){
        const forwarderClient = await ERC20ForwarderClient.factory(provider,feeProxyDomainData,transferHandlerAddress,{apiKey:forwarderApiKey});
        const wallet = provider.getSigner();
        return new TokenWallet(wallet,client);
    }
